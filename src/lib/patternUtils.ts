...
// Generate Gann Wave Chart Data
export const generateGannWaveData = (basePrice: number) => {
  const data = [];
  for (let i = 0; i < 50; i++) {
...
// Generate Elliott Wave Chart Data
export const generateElliottWaveData = (basePrice: number) => {
  const waves = [
  ...
  const data: Array<{ time: number; price: number; waveLabel: string | null; waveType: string }> = [];
